...
if(length(all.equity.curves) != 4) {
  ...
  for(base in 0:3) {
    ...
    all.equity.curves[[base + 1]] <- equity.curve
  }
}
...
matplot(selected$dates, cbind(all.equity.curves[[1]], all.equity.curves[[2]], all.equity.curves[[3]], all.equity.curves[[4]], benchmark), lty = 1, type = "l", xaxt = "n")
...
matplot(selected$dates, cbind(all.equity.curves[[1]] - benchmark, all.equity.curves[[2]] - benchmark, all.equity.curves[[3]] - benchmark, all.equity.curves[[4]] - benchmark), type = "l", main = "Excess return", ylab = "cluster - buy_and_hold")
