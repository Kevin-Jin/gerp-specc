...
get.cluster.leaders <- function(clusters) {
  lag.struct.stats <- lapply(clusters, function(cluster) {
  ...
  attr(cluster.leaders, "leader.stats") <- lapply(1:length(cluster.leaders), function(cluster.index)
  ...
  attr(cluster.leaders, "lag.struct.stats") <- lag.struct.stats
  
  cluster.leaders
}

cluster.leaders <- get.cluster.leaders(predicted.clusters)
group.leaders <- get.cluster.leaders(group.clusters)
